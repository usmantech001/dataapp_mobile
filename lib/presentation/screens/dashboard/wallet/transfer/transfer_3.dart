...
class Transfer3 extends StatelessWidget {
  final User user;
  final num amount;
  const Transfer3({super.key, required this.user, required this.amount});

  @override
  Widget build(BuildContext context) {
  final fee = context.read<GenericProvider>().serviceCharge!.transfer ?? 0;
    return Container(
      height: 530,
      ...
      child: Column(
        children: [
          ...
          CustomContainer(
           ...
           fee <= 0 ? SizedBox.shrink():    CustomKeyValueState(title: "Service Charge", desc: formatCurrency(fee ,code: "NGN")),
              ],
            ),
          ),
          ...
          CustomButton(
            ...
            onTap: () async {
              createTransaction(BuildContext context,
                ...
                try {
                  ServiceTxn _ = await ServicesHelper.inAppTransfer(
              ...
              // await showCustomBottomSheet(
              //     context: context, screen: const TransactionPin());
            },
            loading: false,
          )
        ],
