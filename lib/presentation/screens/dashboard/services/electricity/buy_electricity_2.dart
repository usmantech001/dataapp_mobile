...
import 'arg/electricity_arg.dart';

class BuyElectricity2 extends StatelessWidget {
  final ElectricityArg param;
  const BuyElectricity2({super.key, required this.param});
        ...
        children: [
          ...
          RichText(
            text: TextSpan(
              ...
              children: [
                TextSpan(text: "${formatNumber(param.discount.amount)} "),
                const TextSpan(
                  text: "NGN",
            ...
            child: Column(
              children: [
                CustomKeyValueState(
                    ...
                    desc: param.electricityProvider.name),
                CustomKeyValueState(title: "Meter Number", desc: param.phone),
                CustomKeyValueState(
                    title: "Meter Number", desc: param.meterNumber),
                CustomKeyValueState(title: "Meter Name", desc: param.meterName),
                CustomKeyValueState(
                    title: "Amount", desc: formatCurrency(param.amount)),
                        ...
                        titleW: Row(
                          ...
                          children: [
                            ...
                            Container(
                                ...
                                child: Text(
                                    "${formatDiscountPercentage(param.amount.toDouble(), param.discount.discount?.toDouble() ?? 0)}%",
                                    style: get12TextStyle()
                                        .copyWith(color: ColorManager.kBlack))),
                          ],
                        ),
                        desc: "N ${param.discount.discount}"),
                (param.discount.fee ?? 0) <= 0
                    ? SizedBox.shrink()
                    : CustomKeyValueState(
                        title: "Service Charge",
                        desc: formatCurrency(param.discount.fee)),
                CustomKeyValueState(
                    title: "Total",
          ...
          CustomButton(
            ...
            onTap: () async {
              createTransaction(BuildContext context,
                ...
                try {
                  ServiceTxn _ = await ServicesHelper.buyUnit(
                    provider: param.electricityProvider.code,
                    type: param.meterType.field.toLowerCase(),
                    number: param.meterNumber,
                    amount: param.amount,
                    phone: param.phone,
                    pin: "$transaction_pin",
                  );
