...
import 'arg/airtime_arg.dart';

class BuyAirtime2 extends StatelessWidget {
  final AirtimeArg param;
  const BuyAirtime2({super.key, required this.param});

  @override
  Widget build(BuildContext context) {
    UserProvider userProvider = Provider.of<UserProvider>(context);
    final fee = context.read<GenericProvider>().serviceCharge!.airtime ?? 0;
    //
    return Container(
      height: 530,
        ...
        children: [
          ...
          RichText(
            text: TextSpan(
              ...
              children: [
                TextSpan(
                    text:
                        "${formatNumber(num.parse(param.discount.amount.toString() ?? "0"))} "),
                const TextSpan(
                  text: "NGN",
                  style: TextStyle(
                    fontSize: 15,
          ...
          CustomContainer(
            ...
            child: Column(
              children: [
                CustomKeyValueState(
                    title: "Network", desc: param.airtimeProvider.name),
                CustomKeyValueState(title: "Phone Number", desc: param.phone),
                CustomKeyValueState(title: "Amount", desc: "N ${param.amount}"),
                  ...
                  createTransaction(BuildContext context,
                    ...
                    try {
                      ...
                      await Navigator.pushNamed(
                          ...
                          arguments: _);
                      // Pop back to previous screens as needed
                      // Navigator.pop(context);
                      // Navigator.pop(context);
                      // Navigator.pop(context);
                    } catch (err) {
