            ...
            return Expanded(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  // check if selected tap is cards, the check if cards service is enabled
                  if (e.dashboardTabs == DashboardTabs.cards) {
                    final virtualCard =
                        genericProvider.serviceStatus?.virtualCard;
                    final status = virtualCard?.status ?? false;
                    final value = virtualCard?.value?.toLowerCase();
                    final message = virtualCard?.message?.toLowerCase() ??
                        "This service is not available at the moment.";

                    if (value == "coming-soon" ||
                        value == "temporarily-unavailable") {
                      showCustomToast(context: context, description: capitalizeFirstString(message));
                      return;
                    }

                    if (!status || value == "deactivated") {
                      showCustomToast(context: context, description: message);
                      return;
                    }
                  }
